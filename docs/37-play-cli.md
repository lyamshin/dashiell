...
| `do "<n or words>"` | One choice, by its number on the current list or its words exactly as printed (the label, or the command behind it). Saves, and prints the page it lands on. `do` on "Notebook" prints the notebook; on "File the report" it takes out the form; on "Put it to X" it opens the picker. With `--engine v2` (docs/40 §3) the page lists the people in the room, one line each; `do` on a person opens their topics ("ASK X ABOUT", numbered from 1, with "Put it to X" and "Back to the page"), free and not a page, and `do back` shuts them. A topic can still be taken from the page by its words (`do "ask Rafferty about the third floor"`). |
