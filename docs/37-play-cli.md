...
|---|---|
| `new --seed N --tier 0..5\|over-easy [--level 1..4] [--engine v2] [--name X] --save F` | Deals the night (level defaults to 2, Precinct; Raw always plays at Beat, as in the book), writes the save, and prints the title page's rules, the case line (as the notebook's header has it) and the first page with its choices. |
...
| `grid` | "Where they were", as text: every row against every half hour, the counts, the anchors, the strangers, the key, the rules. |
...
| `link` / `link "<n> <Name>\|none"` | Lists the strangers' sightings, numbered; links one to a person on the grid, or rubs it out. Free, never a fact. |
| `confront "<Name>"` | Opens the "Put it to …" picker for somebody in the room whose account is written down: what they told you, then every fact in the notebook, numbered, under the person or place it is about, with who said it. `do <n>` reads them fact n; the last number is "Put nothing to them". After a fact lands, the page offers "Put another fact to …", free, and `confront` opens it the same way. `confront off` shuts the picker. |
