...
- **Raw and Coddled no longer say "That cleared X" on the oracle's route.** Fixed after review: `clearedOnTwo` (`src/game/m9.ts`) now also needs a fact that is not the person's own word and puts them somewhere other than the scene at the crime's half hour itself (`placedAwayBy`). Before, seed 11 Raw page 7 said "That cleared Donnelly" on Mulcahy's sightings at half past eight and ten, around a crime at half past nine. But Part B's route deals each innocent's corroborating sighting off the crime's half hour on purpose (`teachTheLie` in `src/gen/logic/select.ts`, `f.tick !== M`), so under the stricter rule no Raw page says it at all. Raw still concludes: it names the culprit's lie as a lie. If Raw should teach "That cleared X", the generator needs one corroboration at the crime's half hour; that is `src/gen`'s call. `test/m10-raw.test.ts` and `test/m9-engine.test.ts` are brought to the rule, and `test/m10-testimony.test.ts` builds the seed 11 case and asserts no clear. The Raw and Coddled design targets still hold: marks-follower 15/18/22%, reasoning player 100%.
